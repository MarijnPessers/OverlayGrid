...
namespace OverlayGrid.Controllers
{
    public class HexGridController : IHexGridController
    {
        ...
        private void CalculateRows()
        {
            ...
            Rows = (int)Math.Ceiling(CanvasHeight / _height);
        }
        ...
        private void CalculateColumnCoordinates()
        {
            ...
            for (var row = 0; row < Rows + 2; row += 3)
            {
                var xOffset = CalculateXOffset(row);
                var y_1 = row * _height;
                var startColumn = (row % 2 == 0) ? 0 : -1;
                for (var col = startColumn; col <= Columns; col++)
                {
        ...
        private void CalculateCoordinate1(int row, int col, double x_1, double y_1)
        {
            ...
            SetCoordinate(row - 1, row % 2 == 0 ? col - 1 : col, 3, point);
        ...
        private void CalculateCoordinate2(int row, int col, double x_1, double y_1)
        {
            ...
            SetCoordinate(row - 1, row % 2 == 0 ? col : col + 1, 6, point);
        }
        ...
        private void CalculateCoordinate3(int row, int col, double x_1, double y_1)
        {
            ...
            SetCoordinate(row - 1, row % 2 == 0 ? col : col + 1, 5, point);
            SetCoordinate(row + 1, row % 2 == 0 ? col : col + 1, 1, point);
        }
        ...
        private void CalculateCoordinate6(int row, int col, double x_1, double y_1)
        {
            ...
            SetCoordinate(row + 1, row % 2 == 0 ? col - 1 : col, 2, point);
            SetCoordinate(row - 1, row % 2 == 0 ? col - 1 : col, 4, point);
        }
        ...
        {
            _coordinates = new Dictionary<int, IDictionary<int, IDictionary<int, Point>>>();
